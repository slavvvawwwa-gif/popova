...
9. **Фоновый вектор:** «Настройки сайта» → загрузить SVG и задать прозрачность
   (0–100%). Фон ложится поверх чёрного на правой половине экрана, статичен
   при скролле. Чтобы убрать — удалить файл из поля.
10. **Фильтр по годам** (спектакли/проекты/лаборатории): показываются последние
    5 лет; стрелки ‹ › листают к более ранним / новым датам.
